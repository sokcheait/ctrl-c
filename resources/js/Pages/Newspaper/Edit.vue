...
const form = useForm({
    ...
    des: props.newspaper.des,
    image: props.newspaper.image,
    status: props.newspaper.status,
    menu_id: props.newspaper.menu_id,
});

function updateNewspaper() {
    Inertia.post(`/newspapers/${props.newspaper.id}`, {
            _method: 'put',
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    ...
                    <form class="py-4 px-4" @submit.prevent="updateNewspaper">
                        <div class="relative z-0 mb-6 w-full group">
                            <select name="menu_id" id="menu_id" v-model="form.menu_id" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="select menu">
                                <option v-for="menu in menus" :key="menu.id" :value="menu.id">{{ menu.name }}</option>
                            </select>
                            <label for="menu_id" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">select menus</label>
                        </div>
                        <div class="relative z-0 mb-6 w-full group">
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            <textarea name="des" id="des" cols="30" rows="10" v-model="form.des"
                            ...
                            <label for="des" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Description</label>
                        </div>
                        <div class="relative z-0 mb-6 w-full group">
                            <input type="file" name="file" id="file" @input="form.image = $event.target.files[0]"
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            ...
                            <input type="checkbox" value="" id="status" class="sr-only peer" name="status" v-model="form.status">
